...
export const toCanvasPoint = (point: Point, zoom: number): Point => ({ x: point.x / clampCanvasZoom(zoom), y: point.y / clampCanvasZoom(zoom) });

export const toScreenPoint = (point: Point, zoom: number): Point => ({ x: point.x * clampCanvasZoom(zoom), y: point.y * clampCanvasZoom(zoom) });

export const getScaledCanvasSize = (size: Size, zoom: number): Size => ({ width: size.width * clampCanvasZoom(zoom), height: size.height * clampCanvasZoom(zoom) });
