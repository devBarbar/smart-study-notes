...
const TutorListeningHarness = ({
  ...
  const receiveTutorResponse = () => {
    ...
    setPhase(
      tutorQuestion.assessmentKind === "final_quiz"
        ? "final_quiz"
        : "diagnostic",
    );
  };
...
const RestoredReplayPreview = ({
  ...
  const replay = buildGuidedAudioReplayFromMessage({
    id: "message-1",
    role: "ai",
    text: "Explanation before question.\n\nExplain the idea in your own words?",
    tutorQuestion: buildTutorQuestion(assessmentKind),
  });

  return (
    <View>
      <Text testID="restored-replay-audio">{replay?.text ?? ""}</Text>
    </View>
  );
};
...
Given("a restored guided tutor message", function (this: AppWorld) {
  this.screen = render(<RestoredReplayPreview assessmentKind="guided_notes" />);
});

Given("a restored diagnostic tutor message", function (this: AppWorld) {
  this.screen = render(<RestoredReplayPreview assessmentKind="diagnostic" />);
});
...
Then(
  "the restored replay audio is {string}",
  function (this: AppWorld, audioText: string) {
    assert.equal(
      this.screen!.getByTestId("restored-replay-audio").props.children,
      audioText,
    );
  },
);

Then("no restored replay audio is available", function (this: AppWorld) {
  assert.equal(
    this.screen!.getByTestId("restored-replay-audio").props.children,
    "",
  );
...
Then("the depth check still counts toward mastery", function (this: AppWorld) {
  assert.equal(
    this.screen!.getByTestId("assessment-kind").props.children,
    "depth",
  );
  assert.equal(
    this.screen!.getByTestId("required-for-pass").props.children,
    "true",
  );
});
