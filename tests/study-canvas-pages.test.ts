...
test("canvas page helpers update strokes without touching other pages", () => {
  ...
  const pages = [
    buildInitialCanvasPage("page-1"),
    buildInitialCanvasPage("page-2"),
  ];
...
test("canvas page helpers grow only when drawing near active page edges", () => {
  ...
  const farDownPage = growPageNearEdge(pages, "page-2", {
    x: 100,
    y: pages[1].height + 900,
  });
  assert.equal(farDownPage[1].height, pages[1].height + 900 + 80);
});
