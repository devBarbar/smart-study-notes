...
test('listening notes flow is used for depth tutor questions', () => {
  ...
  assert.equal(
    shouldUseListeningNotesFlow({ ...depthQuestion, assessmentKind: undefined }),
    true,
  );
  assert.equal(
    shouldUseListeningNotesFlow({ ...depthQuestion, assessmentKind: 'guided_notes' }),
    true,
  );
});
...
test('listening notes flow is skipped for diagnostic and final quiz questions', () => {
  assert.equal(
    shouldUseListeningNotesFlow({ ...depthQuestion, assessmentKind: 'diagnostic' }),
    false,
  );
  assert.equal(
    shouldUseListeningNotesFlow({ ...depthQuestion, assessmentKind: 'final_quiz' }),
    false,
  );
...
test('listening notes audio keeps text unchanged when removal is unsafe', () => {
  ...
  assert.equal(getListeningNotesAudioText('Explanation only.', ''), 'Explanation only.');
  assert.equal(
